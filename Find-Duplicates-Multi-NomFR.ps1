<#
.SYNOPSIS
    Trouve les doublons dans la colonne "Nom FR" sur plusieurs fichiers Excel
...
.PARAMETER ColumnName
    Nom de la colonne à analyser (défaut: "Nom FR")

.PARAMETER OutputPath
    Fichier de sortie (optionnel)

.EXAMPLE
    .\Find-Duplicates-Multi-NomFR.ps1 -FilePaths "fichier1.xlsx","fichier2.xlsx"
    .\Find-Duplicates-Multi-NomFR.ps1 -FolderPath "C:\dossier"
#>
...
if ($OutputPath -eq "") {
    if ($FolderPath -ne "") {
        $OutputPath = Join-Path $FolderPath "Doublons_Multi_NomFR.xlsx"
    } else {
        $fileDir = Split-Path -Parent $allFiles[0]
        $OutputPath = Join-Path $fileDir "Doublons_Multi_NomFR.xlsx"
    }
}

Write-Host "========================================" -ForegroundColor Cyan
Write-Host "RECHERCHE DES DOUBLONS MULTI-FICHIERS" -ForegroundColor Cyan
Write-Host "Colonne: NOM FR" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
